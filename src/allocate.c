...
int ethsift_allocate_pyramid(struct ethsift_image pyramid[], uint32_t pyramid_count){
  ...
  if(width / (2 << pyramid_count) <= 0) return 0;
  if(height / (2 << pyramid_count) <= 0) return 0;
  if(__builtin_umul_overflow(width, height, &dim)) return 0;
  ...
  size_t total_size = (4*dim - 2*dim / (2 << 2*(pyramid_count-1))) / 3;
