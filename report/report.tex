...
% The plot will contain several lines, one for each relevant code version. 
% Discuss the plot and extract the overall performance gain from baseline
% to best code. State the percentage of peak performance for the best code.
We achieved huge result. Our results are the best results ever. Noone ever achieved such great results. 

\begin{figure*}[ht]
\centering
  \begin{tikzpicture}
    \begin{axis}[perfDefaults, xmode=log, ymode=log,]
      \addplot+[discard if not={NAME}{ethSIFT avx icc full rdtsc MeasureFull}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {avx};
      \addplot+[discard if not={NAME}{ethSIFT std-c icc full rdtsc MeasureFull}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {stdc};
      \addplot+[discard if not={NAME}{ethSIFT baseline icc full rdtsc MeasureFull}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {base};
    \end{axis}
  \end{tikzpicture}
  \label{fig:full-opts}
...
\centering
  \begin{tikzpicture}
    \begin{axis}[perfDefaults, xmode=log, ymode=log,]
      \addplot+[discard if not={NAME}{ethSIFT avx icc full rdtsc Convolution}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {avx};
      \addplot+[discard if not={NAME}{ethSIFT std-c icc full rdtsc Convolution}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {stdc};
      \addplot+[discard if not={NAME}{ethSIFT baseline icc full rdtsc Convolution}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {base};
    \end{axis}
  \end{tikzpicture}
...
\centering
  \begin{tikzpicture}
    \begin{axis}[perfDefaults, xmode=log, ymode=log,]
      \addplot+[discard if not={NAME}{ethSIFT avx icc full rdtsc GradientAndRotationPyramids}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {avx};
      \addplot+[discard if not={NAME}{ethSIFT std-c icc full rdtsc GradientAndRotationPyramids}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {stdc};
      \addplot+[discard if not={NAME}{ethSIFT baseline icc full rdtsc GradientAndRotationPyramids}]
      table[col sep=comma, x=INPUT-SIZE, y=MEDIAN] {../plots_lib/all.csv} node[above,midway] {base};
    \end{axis}
  \end{tikzpicture}
  \label{fig:grad-opts}
