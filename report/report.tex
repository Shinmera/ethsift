...
\newcommand{\roofPlot}[3]{
  \begin{tikzpicture}
    \begin{axis}[xmode=log, ymode=log,
      ...
      \addplot+[discard if not={NAME}{ethSIFT avx icc full rdtsc #1}]
      table[col sep=comma, x=MEDIAN, y=MEDIAN] {../plots_lib/all.csv}; % TODO x=MEDIAN needs to be replaced with OP-INTENSITY (Extend lisp script(?) with opertational intensity)
      \addplot+[discard if not={NAME}{ethSIFT std-c icc full rdtsc #1}]                         
      table[col sep=comma, x=MEDIAN, y=MEDIAN] {../plots_lib/all.csv};
      \addplot+[discard if not={NAME}{ethSIFT baseline icc full rdtsc #1}]
      table[col sep=comma, x=MEDIAN, y=MEDIAN] {../plots_lib/all.csv};
      ...
      \legend{\strut avx, \strut stdc, \strut base};
    \end{axis}
  \end{tikzpicture}
}
